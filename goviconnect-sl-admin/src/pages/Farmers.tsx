...
export default function Farmers() {
  ...
  const [editForm, setEditForm] = useState({ name: '', email: '', phone: '', district: '' });
  ...
  const handleEdit = (item: Farmer) => {
    setEditItem(item);
    setEditForm({ name: item.name, email: item.email, phone: item.phone || '', district: item.district || '' });
  };
    ...
    <div className="space-y-4">
      ...
      <Modal open={!!editItem} onClose={() => setEditItem(null)} title="Edit Farmer">
        <div className="space-y-3">
          {(['name', 'email', 'phone', 'district'] as const).map((field) => (
            <div key={field}>
